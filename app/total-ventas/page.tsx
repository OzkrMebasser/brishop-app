...
const VendidosPage = () => {
  ...
  const categoriesWithSales = categories.filter((cat) =>
    vendidos.some((p) => p.categoria === cat.id)
  );
  ...
  const filtered = useMemo(
    () =>
      activeCategory === "todos"
        ? vendidos
        : vendidos.filter((p) => p.categoria === activeCategory),
    [activeCategory, vendidos]
  );
  ...
  if (loading) {
    return (
      <main className="container-custom py-24">
        <p className="text-gray-400 text-center py-20">Cargando vendidos...</p>
      </main>
    );
  }
  ...
  >
    Ver Inventario →
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        ...
        >
          Todos <span className="opacity-60">{vendidos.length}</span>
        </button>
        {categoriesWithSales.map((cat) => {
          const count = vendidos.filter((p) => p.categoria === cat.id).length;
          return (
            <button
              key={cat.id}
      ...
      <div className="flex flex-wrap gap-4 mb-10">
        <div className="bg-white border border-gray-200 rounded-xl px-6 py-4 min-w-[150px]">
          ...
          <p className="text-xs text-gray-400 mt-1">
            {activeCategory === "todos"
              ...
              : categoriesWithSales.find((c) => c.id === activeCategory)?.nombre}
          </p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl px-6 py-4 min-w-[150px]">
          <p className="text-xs uppercase tracking-widest text-gray-400 mb-1">Total contado</p>
        ...
        <div className="bg-white border border-gray-200 rounded-xl px-6 py-4 min-w-[150px]">
          <p className="text-xs uppercase tracking-widest text-gray-400 mb-1">Total crédito</p>
              ...
              >
                <div className="shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-gray-100 relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="bg-gray-800/80 text-white text-[9px] font-bold px-1.5 py-0.5 rounded tracking-widest uppercase">
                      Vendido
                    </span>
                  </div>
                </div>
...
// const VendidosPage = () => {
//   const vendidos = products.filter((p) => p.vendido);
//   const [activeCategory, setActiveCategory] = useState<string>("todos");
//   const [currentPage, setCurrentPage] = useState(1);
